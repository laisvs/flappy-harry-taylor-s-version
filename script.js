...
let sound_point = new Audio('sounds effect/hit.mp3');
let sound_die = new Audio('sounds effect/game-over.mp3');
let sound_background = new Audio('sounds effect/background_music.mp3')
...
document.addEventListener('keydown', (e) => {
    ...
    if (key === "Escape") {
        ...
        window.alert("LETS TAKE A BREAK");
    }else{
        ...
        pipe_sprite.forEach((element) => {
            ...
            if(pipe_sprite_props.right <= 0){
                ...
                if(bird_props.left < pipe_sprite_props.left + pipe_sprite_props.width && bird_props.left + bird_props.width > pipe_sprite_props.left && bird_props.top < pipe_sprite_props.top + pipe_sprite_props.height && bird_props.top + bird_props.height > pipe_sprite_props.top || bird_props.top <= 0 || bird_props.bottom >= background.bottom){
                    game_state = 'End';
                    message.innerHTML = 'THIS IS OVER NOW'.fontcolor('red') + '<br><a href="https://open.spotify.com/intl-pt/album/1o59UpKw81iHR0HPiSkJR0">LETS HEAR TAYLORS VERSION</a>'+ '<br>Aperte "Enter" para recomeçar';
                    message.classList.add('messageStyle');
                    img.style.display = 'none';
                    sound_die.play();
                    sound_background.pause();
